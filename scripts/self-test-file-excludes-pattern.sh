#!/usr/bin/env bash
# Self-test для check-file-excludes-pattern.sh.
#
# Сценарии: паттерн отсутствует (rc=0), паттерн есть (rc=1),
...
check_script="${script_dir}/check-file-excludes-pattern.sh"
...
expect_rc "паттерн отсутствует" 0 \
    "$(run_check "${work_dir}/absent.txt" 'forbidden-word')"

printf '%s\n' "has forbidden-word here" > "${work_dir}/present.txt"
expect_rc "паттерн присутствует" 1 \
    "$(run_check "${work_dir}/present.txt" 'forbidden-word')"

expect_rc "файл не найден" 2 \
    "$(run_check "${work_dir}/missing.txt" 'x')"

# Невалидный ERE → grep rc=2.
printf '%s\n' "content" > "${work_dir}/badregex.txt"
expect_rc "невалидный regex" 2 \
    "$(run_check "${work_dir}/badregex.txt" '[')"

echo "[+] self-test для check-file-excludes-pattern.sh passed"
